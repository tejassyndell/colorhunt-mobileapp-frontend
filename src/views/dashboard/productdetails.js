...
function productdetails(props) {
  ...
  const { ProductDatailItem, UserData, selectedprd, serchtext, statusForHart, checkedcetagory, checked, min, max } = props
  ...
  ProductDatailItem.image_128 !== false ? product = `data:image/jpeg;base64,${loaderror === 0 ? ProductDatailItem.image_128 : newImage}` : product = noImages
  ...
  const rmvProductWishlist = async (i) => {
    ...
    const result = await unlinkproductdashboard(data)
    .then((res)=>{
     if(res.status===200){
      if (res.data === true) {
        setheart(false)
  ...
  return (
    <div className="dashboard_product_div">
      {load === true ? <div className="loader-container_profile">
        ...
        <>
          ...
          <div className="editiconproduct ipad_ciontainer">
                  ...
                  {ProductTagColor.map((item) => (
                    <span className='color_radio_man'>
                      <input
                        ...
                        name="categoryFilter"
                        value="all"
                        onClick={() => { setradioColor(item.partner_ref) }}
            ...
            <span className='btn_grp'>
              <div className="quntatity_header">
                <button className='qunt_btn1' onClick={decrementCount}>-</button>
          ...
          <div className={cartstatus === false ? "myCart_exteranal_div" : ""}>
            <div className={cartstatus === true ? "addCart_main_div unactiv_cart" : "addCart_main_div activ_cart"}>
              <div className="cart_header">
                <span className="cart_header_close" onClick={() => { setCart(true) }}
                >X</span>
              </div>
              <div className="checkmart_css">
                {/* <Checkmark size={50} /> */}
                <Checkmark size={50}></Checkmark>
                <h2 className='add_cart_text'>{ProductDatailItem.name} has been added to your cart.</h2>
              </div>
            </div>
          </div>
        </>
