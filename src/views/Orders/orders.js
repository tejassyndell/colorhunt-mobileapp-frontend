    ...
    const FilterApplyBtn = () => {
      ...
      const filterTest = test.filter(item => {
        ...
        if (filter.startDate && filter.endDate) {
          return itemDate >= startFilterDate && itemDate <= endFilterDate || itemDate === endFilterDate;
        }
  ...
  return (
    ...
    <div className="dashboardDiv order_title_header">
      ...
      {FilterOrderData.length > 0 ? (
        <>
          {isOrderHistory === true ? <>
            ...
            <div className="tagdiv tagdiv_order mb-3 mt-3">
              ...
              <i className="fa fa-angle-left" onClick={routeChangeBack} style={{ position: 'absolute' }} aria-hidden="true"></i>
              <div className='tagnames tagname_ord_details'><h5>Order Detail</h5></div>
                ...
                <CTable className="alldriverTable" style={{ tableLayout: 'fixed', textAlign: 'center' }}>
                  <CTableHead>
                    <CTableRow style={{ verticalAlign: 'middel' }} className='tablehadeorder'>
                      ...
                      <CTableHeaderCell className=" drivertablehadding " style={{ width: 65 }}>Quantity</CTableHeaderCell>
                  ...
                  {HistoryFilterData.map((user, index) => (
                    <CTableBody key={index}>
                      <CTableRow style={{ width: '100%', borderBottom: '1px solid #d8dbe0', borderTop: '1px solid #d8dbe0' }}>
                        ...
                        <CTableDataCell className="drivertablerows">{user.price_unit}</CTableDataCell>
                        <CTableDataCell className="drivertablerows" >
                          ₹ &nbsp;{user.price_subtotal}
                        </CTableDataCell>
            ...
            {FilterOrderData.map((item) => (
              ...
              >
                <div className="editicon" style={{ display: 'flex' }} onClick={() => { orderHistory(item.name) }}>
        ...
        <>
          {NoOrderList === true ? <>
            <div className='tagdiv mb-3'><i className="fa fa-angle-left" onClick={routeChange} style={{ position: 'absolute' }} aria-hidden="true"></i> <div className='tagnames'><h5>Order Detail</h5></div>
            {/* <svg className='OrderFilter_Filter_icon' onClick={() => { setFilterDiv(true) }}  width="23" height="22" viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M21.4756 1.85821L1.48523 2.47986L9.77541 11.6866L9.97869 18.2235L14.0389 20.0982L13.7735 11.5623L21.4756 1.85821Z" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
              </svg> */}
              </div>
            <div className='order_empty_div' style={{ height:'80vh' }}> Empty Order List</div>
          </> : <>
      ...
      {FilterDiv === true ? <>
        <div className="OrderFilter_exteranal_div">
          <div className="OrderFilter_main_div">
            ...
            <div>
              <button className='OrderFilter_apply_btn' onClick={() => { FilterApplyBtn() }}>Apply</button>
            </div>
