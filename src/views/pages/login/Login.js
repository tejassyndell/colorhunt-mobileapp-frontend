...
const Login = () => {
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:4000/checkPhone', { phonenumber })
      const data = response.data
      console.log(data)
      if (data.exists) {
        const data = [response.data.Id, response.data.Name]
        localStorage.setItem('Login ID', JSON.stringify(data))
        setisUser(true)
        setOtp(true)
      } else {
                      ...
                      <div className="otpsection">
                        ...
                        <div className="d-flex" style={{ justifyContent: 'center' }}>
                          ...
                          >
                            <CInputGroup
                              className="mb-4"
                              style={{ width: 65, margin: '0px -4px 0px 6px' }}
                            >
                              <CFormInput
                                type="tel"
                                onChange={(e) => {
                            ...
                            <CInputGroup
                              className="mb-4"
                              style={{ width: 65, margin: '0px -4px 0px 0px' }}
                            >
                              <CFormInput
                                type="tel"
                                onChange={(e) => {
                        ...
                        {verifayOtp && <small className="d-block mb-2 red"></small>}
                        <p className="login_otp_countdown">
                          You should recieve the OTP in &nbsp;&nbsp;&nbsp;
                          <span style={{ color: 'rgba(169, 10, 18, 1)' }}>{timer} Seconds</span>
                        </p>
